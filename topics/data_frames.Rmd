...
"Data frame" is R's name for tabular data. We generally want each row in a data frame to represents a unit of observation, and each column to contain a different type of information about the units of observation. Tabular data in this form is called ["tidy data"](http://vita.had.co.nz/papers/tidy-data.html).
...
Under the hood, a data frame is a list of column vectors. This is why `mode` told us that the underlying storage mode of `gap` was list. We can use `$` to retrieve columns, as in a list. (Occasionally it is also useful to use `[[ ]]` to retrieve columns, for example if the column name we want is stored in a variable.)
