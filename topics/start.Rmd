...
![Variables as Sticky Notes](../figures/python-sticky-note-variables-01.png)
...
![Creating Another Variable](../figures/python-sticky-note-variables-02.png)
...
![Updating a Variable](../figures/python-sticky-note-variables-03.png)
...
### Challenge - Indexing and slicing data {.challenge}
...
R has various *functions*, such as `sum( )`. We can get help on `sum` with `?sum`.
...
If you're not sure what type of value you are dealing with you can use `class`. To see how R thinks of a value internally use `mode` (storage mode). We haven't yet seen cases where `class` and `mode` differ, but we will in the next section. For more detailed information use `str` (structure). Try the following:
