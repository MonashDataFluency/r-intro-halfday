...
If you're not sure what type of value you are dealing with you can use `class`. To see how R thinks of a value internally use `mode` (storage mode). We haven't yet seen cases where `class` and `mode` differ, but we will in the next section. For more detailed information `str` (structure). Try the following:
